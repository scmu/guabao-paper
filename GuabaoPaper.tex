...
choosing \ensuremath{\Varid{a}\times\Varid{b}\mathbin{+}\Varid{r}\mathrel{=}\Conid{A}\times\Conid{B}} as the loop invariant (line 7), which can be established by initialisation \ensuremath{\Varid{a},\Varid{b},\Varid{r}\mathbin{:=}\Conid{A},\Conid{B},\mathrm{0}} (line 6).
...
The PO labelled {\sf TermBase}, however, turns out to be falsifiable. Indeed, the premise does not guarantee \ensuremath{\Varid{b}\geq \mathrm{0}}! We thus realise that we need a stronger invariant. The new invariant would be:
\begin{hscode}\linenumsetup\printlinebegin\SaveRestoreHook
\column{B}{@{}>{\hspre}l<{\hspost}@{}}%
...
The postcondition on line 17 formally describes the goal --- notice the use of Eindhoven notation for quantification, which denotes ``for \ensuremath{\mathrm{0}\leq \Varid{i}\leq \Varid{j}\leq \Conid{N}}, collect \ensuremath{\Varid{sum}\;\Varid{i}\;\Varid{j}} and find the largest \ensuremath{(\mathbin{\uparrow})},  where \ensuremath{\Varid{i}} and \ensuremath{\Varid{j}} are bound variables.''
Guabao supports arrays that can be nested and mutable (if declared in \ensuremath{\mathbf{var}}).
Pure functions to be used in assertions can be defined in declaration blocks \ensuremath{\{\mskip1.5mu \mathbin{:}\mathbin{...}\mathbin{:}\mskip1.5mu\}} (line 5 -- 7 in Figure~\ref{fig:mss}).
...
\printlineend\\[\blanklineskip]%
\printlinebegin\>[B]{}\lBrace\Conid{P}\rBrace\,\Varid{ss}\;\{\mskip1.5mu \Conid{R}\mskip1.5mu\}\;\Conid{Ss}\,\lBrace\Conid{Q}\rBrace{}\<[27]%
\>[27]{}\mathrel{=}\lBrace\Conid{P}\rBrace\,\Varid{ss}\,\lBrace\Conid{R}\rBrace;\lBrace\Conid{R}\rBrace\,\Conid{Ss}\,\lBrace\Conid{Q}\rBrace{}\<[62]%
\>[62]{}\label{code:struct:seq:2}{}\<[E]%
\printlineend\\
...
\printlineend\\
\printlinebegin\>[30]{}\Varid{tellSpec}\;[\mskip1.5mu \mathbin{!}\Conid{P'},\Conid{Q'}\mathbin{!}\mskip1.5mu]{}\<[E]%
\printlineend\\[\blanklineskip]%
