...
We declare two constants \ensuremath{\Conid{A}} and \ensuremath{\Conid{B}}, about them all we know is that they are both non-negative (as asserted in \ensuremath{\assert{\Conid{A}\geq \mathrm{0}\mathrel{\wedge}\Conid{B}\geq \mathrm{0}}}).
...
The goal of the program is to store the value of \ensuremath{\Conid{A}\times\Conid{B}} in variable \ensuremath{\Varid{r}}, as stated in the postcondition \ensuremath{\assert{\Varid{r}\mathrel{=}\Conid{A}\times\Conid{B}}}.
The question mark indicates code yet to be written.
\begin{hscode}\linenumsetup\printlinebegin\SaveRestoreHook
\column{B}{@{}>{\hspre}l<{\hspost}@{}}%
\column{E}{@{}>{\hspre}l<{\hspost}@{}}%
\>[B]{}\mathbf{con}\;\Conid{A},\Conid{B}\mathbin{:}\Conid{Int}\;\assert{\Conid{A}\geq \mathrm{0}\mathrel{\wedge}\Conid{B}\geq \mathrm{0}}{}\<[E]%
\printlineend\\
...
The interface shows, on line 5 and 7, that code to be filled in shall bring the state of the system from precondition \ensuremath{\Conid{True}} to postcondition \ensuremath{\Varid{r}\mathrel{=}\Conid{A}\times\Conid{B}}. Properties of global constants (namely \ensuremath{\Conid{A}\geq \mathrm{0}\mathrel{\wedge}\Conid{B}\geq \mathrm{0}}) are universally true and implicitly conjuncted with all assertions. They are displayed separately in a ``Property'' section in the right pane.
...
Once we start doing the proofs, it immediately turns out that we cannot prove {\sf TermBase} --- the premise does not guarantee \ensuremath{\Varid{b}\geq \mathrm{0}}! We thus realise that we need a stronger invariant. The new invariant would be
\begin{hscode}\linenumsetup\printlinebegin\SaveRestoreHook
\column{B}{@{}>{\hspre}l<{\hspost}@{}}%
...
Bottom: trying to fill in \ensuremath{\Varid{b}\mathbin{:=}\Varid{b}\times\mathrm{2}} in the loop body. It will turn out that we cannot prove {\sf TermInd}.}
\label{fig:sshot45}
\end{figure}
...
Another possible choice is \ensuremath{\Varid{a}\mathbin{:=}\Varid{a}\times\mathrm{2}}.
...
\printlineend\ColumnHook
\end{hscode}\resethooks
which, under integral division, can be true \emph{provided that \ensuremath{\Varid{b}} is an even number}.
...
\paragraph{Totalising \ensuremath{\mathbf{if}}}
...
\printlineend\\
\printlinebegin\>[B]{}\Varid{wp}\;(\mathbf{do}\;\Conid{B}_{i}\to \Conid{S}_{i}\;\mathbf{od})\;{}\<[24]%
\>[24]{}\Conid{Q}\mathrel{=}\mathbin{??}{}\<[E]%
\printlineend\\[\blanklineskip]%
\printlinebegin\>[B]{}\Varid{wp}\;\epsilon\;{}\<[13]%
\>[13]{}\Conid{Q}\mathrel{=}\Conid{Q}{}\<[46]%
\>[46]{}\label{code:wp:seq:0}{}\<[E]%
\printlineend\\
...
Main programs in Guabao also come in the form \ensuremath{\htriple{\Conid{P}}{\Conid{S}}{\Conid{Q}}}.
...
we emit two POs: \ensuremath{\Conid{R}\mathrel{\Rightarrow}\Varid{wp}\;\Conid{S}_{2}\;(\Varid{wp}\;\Conid{S}_{3}\;\Conid{Q})}, and
\ensuremath{\Conid{P}\mathrel{\Rightarrow}\Varid{wp}\;\Conid{S}_{0}\;(\Varid{wp}\;\Conid{S}_{1}\;\Conid{R})}.
That is, we assume that \ensuremath{\Conid{R}}, an assertion intentionally left there by the programmer, represents all what the programmer wishes to establish at this point and contains all the information about the current state needed to prove the program correct.
...
\printlineend\\
\printlinebegin\>[B]{}\hsindent{3}{}\<[3]%
\>[3]{}\Varid{termInd}\;\Conid{P}\;\Varid{e}\;\Conid{B}_{0}\;\Conid{S}_{0};\Varid{termInd}\;\Conid{P}\;\Varid{e}\;\Conid{B}_{1}\;\Conid{S}_{1}{}\<[E]%
\printlineend\\
\printlinebegin\>[B]{}\label{code:struct:do:3}{}\<[E]%
\printlineend\\[\blanklineskip]%
...
There is one final issue. \todo{explain \ensuremath{\Varid{termInd}}.}
